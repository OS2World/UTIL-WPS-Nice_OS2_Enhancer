...
:p.
В OS/2 есть путаница с именами файлов. Дело в том, что если создать файл на рабочем столе
с помощью WPS, то он кроме настоящего имени получает еще одно, которое будет использовано
для его значка как заголовок. Вроде бы все в порядке, но если затем перенести этот файл на
диск и переименовать его с помощью FM/2, File Commander, Larsen Commander или командой
"Rename", то заголовок значка не изменяется. Таким образом, можно получить файл, который
в WPS и File Manager будет называться по-разному.
...
:p.
То же самое. Каждый файл может иметь свой значок. Все хорошо, но его можно изменить также
...
:p.
Отключается свойство "Только для чтения", получаемое при копировании файлов с CD/DVD.
