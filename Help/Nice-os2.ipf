...
:p.
Other information about Nice Enhancer can be found in other parts of this manual. 
Click "Help" button in settings page to open manual for the selected page.
...
is turned "ON".
:p.
This feature requires native OS/2 scrollbars so it does not work in FireFox. 
...
:p.
The Workplace Shell allows you to operate your documents as usual objects - paper,
books, pictures, and more. Now you can expand the Desktop by four "rooms".
...
and detached (internal system) applications worls with their own levels.
...
следует вызывать в "Startup.cmd" (или до его вызова надо вызвать SysWaitForShell и
...
:p.
Дополнительные сведения о том, как работает Nice и всех его настройках вы найдете в
других главах этого руководства.
...
захватить движок полоски, удерживая нажатой кнопку мыши, и передвинуть мышь, то
изображение будет плавно двигаться в другую сторону. Обычно это выполняется
...
в окне. Теперь мыши могут работать правильно во всех случаях.
...
и включен "Scroll Lock" или "Caps Lock". Изображение следует за указателем мыши, 
который при этом принимает вид руки.

:p.
Чтобы эти возможности можно было использовать, приложение должно создать настоящие
"осевые" полоски просмотра. Если их нет, передвигать изображение будет нельзя.
...
:p.
Рамки и заголовки всех окон можно перерисовывать так, чтобы подчеркнуть их объем.
...
в природе. Белого цвета здесь тоже нет - в чем же дело?
...
:artwork align='center' name='Images\Eye.bmp'.
:p.
Вы сами можете дать ответ на вопрос, как мы видим, например, желтый цвет - это смесь
красного и зеленого. Голубой цвет для человека - смесь зеленого и синего, а фиолетовый -
синего и красного. Белый и серый можно получить, если смешать все цвета сразу.

:p.
Компьютеры работают точно так же. Зачем показывать настоящий оранжевый цвет, когда можно
взять красный и добавить немного зеленого? Человек все равно увидит оранжевый. Никто
не может отличить настоящий цвет от смешанного &colon.-)
...
человеком, зависят сразу от трех составляющих, и чтобы показать их все сразу, необходимо
трехмерное пространство&colon.
.br
:artwork align='center' name='Images\RGBCube.bmp'.
:p.
...
:p.
Используя разные цветовые решения вы можете раскрасить OS/2 как вам нравится. Выбирайте
цвета, которые вы видите каждый день, и избегать слишком пестрой раскраски - от нее
устают глаза.

:p.
Меню и цвет выделения для списков можно сделать темно-серым. Это "нейтральный" цвет, в
нем равное количество красного, зеленого и синего цвета.
...
уменьшить расход памяти и избежать некоторых ошибок.
...
:p.
Оболочка WPS позволяет вам работать с документами как с обычными предметами -
листками бумаги, книгами и так далее. Вы можете расширить рабочее пространство,
добавить к рабочему столу несколько "комнат".
...
или LaunchPad, а также переносить в нее работающие приложения, выбирая их из
списка окон.
...
использовать значки комнат наповерхности eCenter или WarpCenter&colon.
.br
:artwork align='center' name='Images\Rooms.bmp'.
:p.
Расширитель crhsdftn и показывает те окна, у которых есть команда "Свернуть" 
...
если вы поддерживаете несколько Интернет-сайтов, то Nice предполагает, что для каждого
из них будет вызываться отдельное окно SSH со своим заголовком, поэтому для каждого из
них надо составить текстовый файл с командами и заголовком этого окна (или несколькими
возможными заголовками). Файлы желательно располагать в каталогах, где содержатся данные,
используемые в работе (документы, скрипты и т.д.), а не в каталоге расширителя.

:p.
Чтобы вызывать SSH с разными заголовками, создайте на рабочем столе разные значки для
вызова SSH - подпись под значком станет заголовком окна. Можно вызвать SSH с определенным
...
:p.
Для некоторых клавиш можно назначить определенные действия. Вы можете вызывать
...
MP3-проигрывателя или сетевого обозревателя и управление ими. Если на клавиатуре есть
ряд из восьми кнопок "easy access", разделенных как (3+2+3), их можно использовать для
вызова FC/2, Mozilla и так далее.

:p.
Расширитель позволяет определить клавиши Ctrl + C / V для того, чтобы запоминать и
вставлять текст в оконных приложениях. Вначале они использовались на компьютерах Apple,
но могут быть полезны и в OS/2. Они заменяются на обычные Ctrl / Shift + Insert.
...
:p.
:color fc=yellow bc=default.:color fc=default bc=default.
Удерживать Num Lock включенным или выключенным.

:p.
Этот переключатель можно удерживать в определенном состоянии все время.
Также можно переопределить цифровые клавиши как обычные "1..9", чтобы они
всегда набирали цифры. Это полезно для некоторых приложений, перенесенных с
Unix.
...
Все бы хорошо, но для этого используется клавиша "Enter". Эту возможность можно
...
:p.
При каждом нажатии на клавишу будет раздаваться ужасный звук, который записан в
файле "Keyboard.wav" в каталоге расширителя.
...
:p.
Правая и средняя кнопки мыши могут быть использованы для действий с полосками просмотра
и заголовками окон. Это сделано прежде всего сетевого обозревателя Mozilla и приложений,
работающих в среде ODIN.
...
:p.
Перенос значков ("Drag and drop") таким образом переопределить нельзя, только нажатия на
кнопки.
...
:p.
Если включить эту настройку, двойное нажатие на системное меню (картинку в левом верхнем
углу окна) будет вызывать меню вместо закрытия окна.
...
:p.
Расширитель позволяет выбирать строки в текстовых окнах с помощью мыши и сразу же
запоминать выбранный текст. Кроме того, поддерживаются действия "запомнить" и "вставить",
выполняемые при нажатии на клавиши Ctrl + Insert и Shift + Insert.

:p.
:color fc=yellow bc=default.:color fc=default bc=default.
Использовать среднюю кнопку чтобы запоминать и вставлять текст
...
мыши. Ввод от клавиатуры может быть перенесен в окно, расположенное под указателем мыши.
...
его, не набирая в нем никаких значений. Затем вызовите настройки расширителя и
...
:p.
Например, если при запуске OS/2 вы осуществляете вход в сеть Novell и хотели бы
передать имя и пароль вашему скрипту на Rexx, который выполнит подключение к
другим сетевым службам, откройте окно NWTools, не набирая в нем имя и пароль,
...
Расширитель запомнит окно и значения полей ввода в нем, и будет восстанавливать
содержимое полей ввода в нем каждый раз при его появлении.
...
:p.
Приоритет приложения или потока - это некоторое значение, которое определяет, сколько
времени ему будет дано процессором. Значения приоритетов умеет показывать WatchCat,
для этого надо вызвать в нем окно "Threads".
...
:p.
Еще одно решение - использовать процессор, поддерживающий Hyper-Threading или вообще
перейти на SMP (или и то и другое одновременно).
...
работает с ее собственным приоритетом.
...
:p.
Если вы используете приложения, окна которых необязательно показывать в списке окон,
вы можете удалить из списка окон несколько строк.
...
:p.
Если эта настройка включена, Nice будет восстанавливать скрытые и уменьшенные окна перед
их закрытием. Это полезно для приложений, которые запоминают расположение и состояние
своих окон и не показывают их при запуске, если они оставались невидимыми.
