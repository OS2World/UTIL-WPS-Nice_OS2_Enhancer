...
#include "Modules\\Arranger\\Arranger_scan.cpp"
...
// Update_windows - надо ли обновить все окна, Arrange_windows - надо ли их выравнивать.
VOID EXPENTRY Krnl_ReadSettings (BYTE Update_windows, BYTE Arrange_windows)
{
  // Открываем файл настроек.
  CHAR Settings_file_name[SIZE_OF_PATH] = ""; GetSettingsFileName (Settings_file_name);
  HINI Ini_file = OpenIniProfile (Enhancer.Application, Settings_file_name);
  ...
  // Выравниваем некоторые окна.
  if (Arrange_windows) ArrangeSomeWindows (Desktop);
