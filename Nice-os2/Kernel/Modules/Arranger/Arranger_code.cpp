...
VOID Arranger_ArrangeWindows (ULONG Action, HWND Frame_window)
{
  ...
  // Выравниваем окна сетевых соединителей.
  if (Action == SM_ARRANGE_NW_DIALER)
  {
    // Задаем новый размер и расположение окна.
    INT X_Size = X_Screen / 2;
    INT Y_Size = Y_Screen / 16;
    INT X_Position = (X_Screen - X_Size) - 1;
    INT Y_Position = 30;
  ...
  // Выравниваем окна XWorkplace, в том числе Pager.
  if (Action == SM_ARRANGE_XWP_PAGER)
  {
    // Задаем новый размер и расположение окна.
    INT X_Size = Window_placement.cx;
    INT Y_Size = Window_placement.cy;
    INT X_Position = 1;  if (Window_placement.cx > Window_placement.cy * 3) X_Position = (X_Screen - X_Size) - 1;
    INT Y_Position = 30; if (Window_placement.cx > Window_placement.cy * 3) Y_Position = 30;
