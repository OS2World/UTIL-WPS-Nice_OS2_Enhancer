{
  ...
  // Заполняем список действий.
  {
    ...
    if (Code_page == RUSSIAN)
    {
      ...
      Point ++; if (Point < MAX_ACTIONS) strcpy (Strings[Point], "ArcaOS TaskBar");
      Point ++; if (Point < MAX_ACTIONS) strcpy (Strings[Point], "ArcaOS Pager");
   ...
   // Задаем список действий.
   {
     ...
     Point ++; if (Point < MAX_ACTIONS) Keyboard_Actions.Settings.Actions.Actions[Point] = SHOW_XWP_TASKBAR;
     Point ++; if (Point < MAX_ACTIONS) Keyboard_Actions.Settings.Actions.Actions[Point] = SHOW_XWP_PAGER;
