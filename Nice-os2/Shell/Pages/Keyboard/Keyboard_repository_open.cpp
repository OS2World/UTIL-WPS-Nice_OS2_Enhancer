...
VOID Keyboard_Repository_CreatePage( VOID )
{
 // Указатель на страницу.
 PPAGE Page = Enhancer.Pages.Keyboard_repository;

 // Создаем страницу.
 CHAR Page_class_name[] = "Keyboard_Repository_WndClass";
 WinRegisterClass( Enhancer.Application, Page_class_name, (PFNWP) Keyboard_Repository_WndProc, 0, 0 );
...

 {
  ...
  Checkbox_brick.hwnd = WinCreateWindow( Page->Window, WC_BUTTON, NULL, BS_CHECKBOX, LTM_X, LTM_Y, LTM_CX, LTM_CY, Page->Window, HWND_TOP, Keyboard_Repository.Settings.CheckWPS_button_ID, NULL, NULL );
  ...
  if( Code_page == RUSSIAN ) WinSetWindowText( Checkbox_brick.hwnd, StrConst_RU_Pages_Keyboard_repository_CheckWPS );
  else WinSetWindowText( Checkbox_brick.hwnd, StrConst_EN_Pages_Keyboard_repository_CheckWPS );
 ...
 if( Code_page == RUSSIAN )
  CreatePromptText( Page, StrConst_RU_Pages_Keyboard_repository_Prompt );
 else
  CreatePromptText( Page, StrConst_EN_Pages_Keyboard_repository_Prompt );
