...
MRESULT EXPENTRY Placement_Settings_WndProc (HWND Window, ULONG Message, MPARAM First_parameter, MPARAM Second_parameter)
{
  ...
  switch (Message)
  {
    ...
    case SM_SHOW_SETTINGS:
    {
      ...
      Value = 0; if (Arranger.Settings.Arrange_Network_windows) Value = 1;
      WinSendDlgItemMsg (Window, Placement_Settings.Settings.Network_button_ID, BM_SETCHECK, MPFROMLONG (Value), 0);

      Value = 0; if (Arranger.Settings.Arrange_XWP_windows) Value = 1;
      WinSendDlgItemMsg (Window, Placement_Settings.Settings.XWP_button_ID, BM_SETCHECK, MPFROMLONG (Value), 0);
    }
    return 0;
    ...
    case WM_CONTROL:
    {
      ...
      if (WM_Control_Window_ID == Placement_Settings.Settings.Network_button_ID)
      {
        switch (WM_Control_Action_ID)
        {
          case BN_CLICKED:
          case BN_DBLCLICKED:
          {
            ...
            if (Button_is_checked) Arranger.Settings.Arrange_Network_windows = 0;
            else Arranger.Settings.Arrange_Network_windows = 1;
      ...
      if (WM_Control_Window_ID == Placement_Settings.Settings.XWP_button_ID)
      {
        switch (WM_Control_Action_ID)
        {
          case BN_CLICKED:
          case BN_DBLCLICKED:
          {
            ...
            if (Button_is_checked) Arranger.Settings.Arrange_XWP_windows = 0;
            else Arranger.Settings.Arrange_XWP_windows = 1;
    ...
    case WM_COMMAND:
    {
      ...
      if (WM_Control_Button_ID == OK_BUTTON_ID)
      {
        ...
        if (Ini_file)
        {
          ...
          PrfWriteProfileData (Ini_file, "Settings", "Arrange Browser windows", &Arranger.Settings.Arrange_Browser_windows, sizeof (BYTE));
          PrfWriteProfileData (Ini_file, "Settings", "Arrange Network windows", &Arranger.Settings.Arrange_Network_windows, sizeof (BYTE));
          PrfWriteProfileData (Ini_file, "Settings", "Arrange XWP windows",     &Arranger.Settings.Arrange_XWP_windows,     sizeof (BYTE));
