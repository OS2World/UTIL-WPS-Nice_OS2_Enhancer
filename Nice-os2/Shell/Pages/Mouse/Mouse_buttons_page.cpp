...
MRESULT EXPENTRY Mouse_Buttons_WndProc (HWND Window, ULONG Message, MPARAM First_parameter, MPARAM Second_parameter)
{
  ...
  switch (Message)
  {
    ...
    {
      BYTE Value = 0; if (MouseMapper.Settings.Use_right_button_in_scrollbars) Value = 1;
      WinSendDlgItemMsg (Window, Mouse_Buttons.Settings.ScrollBars_ID, BM_SETCHECK, MPFROMLONG (Value), 0);

      Value = 0; if (MouseMapper.Settings.Invert_mouse_buttons_with_Caps_Lock) Value = 1;
      WinSendDlgItemMsg (Window, Mouse_Buttons.Settings.CapsLock_ID, BM_SETCHECK, MPFROMLONG (Value), 0);
    }
    return 0;
    ...
    case WM_CONTROL:
    {
      ...
      if (WM_Control_Window_ID == Mouse_Buttons.Settings.ScrollBars_ID)
      {
        switch (WM_Control_Action_ID)
        {
          ...
          case BN_DBLCLICKED:
          {
            ...
            if (Button_is_checked) MouseMapper.Settings.Use_right_button_in_scrollbars = 0;
            else MouseMapper.Settings.Use_right_button_in_scrollbars = 1;
      ...
      if (WM_Control_Window_ID == Mouse_Buttons.Settings.CapsLock_ID)
      {
        switch (WM_Control_Action_ID)
        {
          case BN_CLICKED:
          case BN_DBLCLICKED:
          {
            ...
            if (Button_is_checked) MouseMapper.Settings.Invert_mouse_buttons_with_Caps_Lock = 0;
            else MouseMapper.Settings.Invert_mouse_buttons_with_Caps_Lock = 1;
    ...
    case WM_COMMAND:
    {
      ...
      if (WM_Control_Button_ID == OK_BUTTON_ID)
      {
        ...
        if (Ini_file)
        {
          PrfWriteProfileData (Ini_file, "Settings", "Use right button in scrollbars", &MouseMapper.Settings.Use_right_button_in_scrollbars, sizeof (BYTE));
          PrfWriteProfileData (Ini_file, "Settings", "Invert mouse buttons with Caps Lock", &MouseMapper.Settings.Invert_mouse_buttons_with_Caps_Lock, sizeof (BYTE));
