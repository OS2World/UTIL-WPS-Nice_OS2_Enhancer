...
{
  // Если указатель не задан - длина равна нулю.
...
{
  // Если один из указателей не задан - ничего не делаем.
...
{
  // Если указатель не задан или длина равна нулю - ничего не делаем.
...
{
  // Если один из указателей не задан или длина равна нулю - ничего не делаем.
  if (Target == NULL) return;
  if (Source == NULL) return;
  if (Length <= 0) return;
...
{
  // Если указатель на строку не задан - ничего не делаем.
  // Код буквы может быть равен нулю (такое бывает при подготовке списков строк).
...
{
  // Если указатель на строку не задан или границы заданы неправильно - ничего не делаем.
  if (String == NULL) return;
  ...
  // Выполняем замену.
  {
    ...
    while (String[Count] != 0)
    {
      ...
      {
        Strings_strcpy (&String[Count], &String[Count + 1]); continue;
      }
